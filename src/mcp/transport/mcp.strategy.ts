 ...
 * binds every `@Tool`/`@Resource`/`@Prompt` handler (scoped to {@link MCP_TRANSPORT})
 * into this strategy's `messageHandlers`. On `listen()` the strategy reads each
...
export class McpStrategy extends Server implements CustomTransportStrategy {
  public override transportId = MCP_TRANSPORT;
  ...
  constructor(public readonly options: McpServerOptions) {
    ...
    this.logger = createMcpLogger(McpStrategy.name, this.options as any);
  }
