...
#
# NOTE: these are throwaway dev credentials baked into the playground seed.
# Do NOT use these values anywhere real.
set -euo pipefail

CASDOOR_URL="${CASDOOR_URL:-http://localhost:8000}"
CLIENT_ID="${CLIENT_ID:-mcp-playground-client}"
CLIENT_SECRET="${CLIENT_SECRET:-mcp-playground-secret-dev-only}"
