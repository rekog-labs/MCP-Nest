...
/**
 * External-Auth MCP playground — the RESOURCE SERVER.
 *
 * The MCP server is a NestJS microservice transport strategy (`McpStrategy`),
 * NOT an `McpModule`. It reuses the shared greeting capability classes and adds
 * a `WellKnownController` advertising self-hosted Casdoor (docker-compose.yml)
...
@Module({
  controllers: [
    ...
    McpHttpController,
    WellKnownController,
    // @McpController() capability classes (RPC handlers, not HTTP routes).
    GreetingTool,
    GreetingResource,
    GreetingPrompt,
  ],
