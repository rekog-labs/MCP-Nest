/**
 * Shared Casdoor access-token verification used by BOTH auth styles in this
 * example:
 *
 * - `casdoor-auth.guard.ts`   — the recommended path (a NestJS guard on a real
 *                               MCP controller), and
 * - `casdoor-jwt.middleware.ts` — the alternative path (Express middleware),
 *                               kept for reference.
 *
